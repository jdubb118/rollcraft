...
export function resolveSubmissionPhase(
  ...
  if (isChained && phase === 1) attackRoll += 10;
  ...
  if (threshold > 30) {
    return { escaped: false, tapped: true, advancePhase: false, attackerStaminaCost, defenderStaminaCost,
    ...
    return { escaped: false, tapped: false, advancePhase: true, attackerStaminaCost, defenderStaminaCost,
      message: `${move.name} tightening... Phase ${phase + 1}!` };
  } else if (threshold >= 0) {
    if (phase >= 3) {
      return { escaped: true, tapped: false, advancePhase: false, attackerStaminaCost, defenderStaminaCost,
